...
import { Sprout, Package, TrendingUp, ClipboardList, User } from 'lucide-react'
...
const tabs: TabItem[] = [
  { label: 'Dashboard', icon: Sprout, path: '/dashboard-petani' },
  { label: 'Produk', icon: Package, path: '/upload-produk' },
  { label: 'Harga', icon: TrendingUp, path: '/harga-pasar' },
  { label: 'Pesanan', icon: ClipboardList, path: '/pesanan-petani' },
  { label: 'Profil', icon: User, path: '/profil' },
]

export function FarmerBottomTabBar() {
  const location = useLocation()
  const navigate = useNavigate()

  const isActive = (path: string) => {
    if (path === '/dashboard-petani') {
